...
export type SeedSource = 'random' | 'custom';
...
const empty = (): Records => ({ random: [], custom: [] });
...
export function saveRecord(r: CareerRecord): Records {
  ...
  const board = r.seedSource === 'custom' ? 'custom' : 'random';
...
export function rankOf(records: Records, r: CareerRecord): number {
  const board = records[r.seedSource === 'custom' ? 'custom' : 'random'];
